...
import { X, CheckCircle2, AlertTriangle, Info, AlertCircle, Navigation } from 'lucide-react'
...
function App() {
  ...
  // Hackathon Demo Mode background worker
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (state.demoMode) {
        ...
        // 12% chance of new arrival simulation
        if (rng < 0.12) {
          state.simulateNewArrival()
        } 
        // 12% chance of item movement scan
        else if (rng < 0.24) {
          const shelvesList = ['A1', 'A2', 'A3', 'B1', 'B2', 'B3', 'F1', 'F2']
          const shelf = shelvesList[Math.floor(Math.random() * shelvesList.length)]
          const isCorrect = Math.random() > 0.2
          state.simulatePlacement(shelf, isCorrect)
        } 
        // 8% chance of triggering AI recommendations
        else if (rng < 0.32) {
          state.simulateAIRecommendation()
        }
        // 6% chance of timeout error escalation on a pending shelf
        else if (rng < 0.38) {
          const pendingShelves = Object.values(state.shelves).filter(s => s.status === 'pending')
          if (pendingShelves.length > 0) {
            const chosen = pendingShelves[Math.floor(Math.random() * pendingShelves.length)]
            state.simulateTimeout(chosen.id)
          }
        }
      }
    }, 4500)
      ...
      <main className="flex-1 h-full flex flex-col relative overflow-hidden bg-[#0B0B0D]">
        ...
        <header className="h-11 px-6 border-b border-[#22252C] flex items-center justify-between shrink-0 bg-[#121317]">
          ...
          <div className="flex items-center space-x-3 text-xs">
            <span className="text-zinc-500 font-medium">Node latency:</span>
            <span className="font-mono text-[#FF6B35] font-bold">
              {useInvenioStore(state => state.latency)}ms
            </span>
          </div>
        </header>
          ...
          {useInvenioStore(state => state.notifications).map((notif) => (
            ...
            >
              <div className="shrink-0 mt-0.5">
                ...
                {notif.type === 'error' && <AlertCircle className="w-5 h-5 text-rose-500" />}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-zinc-100">{notif.message}</p>
                <span className="text-[10px] text-zinc-500 font-mono mt-1 block">{notif.timestamp}</span>
              </div>
              <button 
                onClick={() => useInvenioStore.getState().dismissNotification(notif.id)}
          ...
          <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-[11px] text-zinc-400 leading-relaxed font-medium">
                The camera will automatically lock on to the vehicle's spatial coordinates and track its pathfinding route in real-time across the warehouse floor.
              </p>
