...
import { 
  Search, Terminal, Database, ShieldAlert, Settings, 
  LayoutDashboard, Map, List, BarChart3, HelpCircle 
} from 'lucide-react'
import { useInvenioStore, type PageRoute } from '../store/store'
...
export const CommandPalette: React.FC = () => {
  ...
  const baseCommands: CommandItem[] = [
    ...
    { 
      icon: Settings, 
      label: 'Open Settings parameters', 
      shortcut: 'G S', 
    ...
    { 
      icon: Terminal, 
      label: `${demoMode ? 'Pause' : 'Resume'} Simulation Mode`, 
      shortcut: 'T D', 
      action: () => {
        setDemoMode(!demoMode)
        addCustomAlert('info', `Simulation has been ${!demoMode ? 'resumed' : 'paused'}.`)
        setIsOpen(false)
      }, 
      category: 'Utilities' 
    }
          ...
          >
            ...
            <div className="flex items-center px-4 py-3.5 border-b border-[#22252C]">
              ...
              <input
                ref={inputRef}
                type="text"
                placeholder={isLocateMode ? "Search inventory item by name or SKU..." : "Type a command or search..."}
                value={search}
            ...
            {/* List area */}
            <div className="max-h-[320px] overflow-y-auto p-2 bg-[#121317]">
              {filtered.length === 0 ? (
                    ...
                    {filtered.map((el, index) => {
                      ...
                      if (el.isItem) {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center">
                              <Database className={`w-4 h-4 mr-3 ${active ? 'text-[#FF6B35]' : 'text-zinc-550'}`} />
                              <div>
                                <span className={`text-xs font-medium ${active ? 'text-white' : 'text-[#A1A1AA]'}`}>
                                  {item.name}
                                </span>
                                <span className="text-[10px] text-zinc-500 ml-2 font-mono">{item.sku}</span>
                              </div>
                            </div>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center">
                              <Icon className={`w-4 h-4 mr-3 ${active ? 'text-white' : 'text-zinc-500'}`} />
                              <span className={`text-xs font-medium ${active ? 'text-white' : 'text-[#A1A1AA]'}`}>
                                {cmd.label}
                              </span>
                            </div>
            ...
            <div className="flex items-center justify-between px-4 py-2 border-t border-[#22252C] bg-[#0B0B0D] text-[9px] text-zinc-500">
              <div className="flex items-center space-x-2">
                <span>Navigate:</span>
                <span className="bg-[#121317] px-1 py-0.5 rounded border border-[#22252C]">↑↓</span>
                <span>Select:</span>
                <span className="bg-[#121317] px-1 py-0.5 rounded border border-[#22252C]">Enter</span>
              </div>
              {isLocateMode && (
                <div>
                  <span>Press <span className="bg-[#121317] px-1 py-0.5 rounded border border-[#22252C] font-mono">Backspace</span> to go back</span>
                </div>
              )}
              {!isLocateMode && (
                <div>
                  <span>Press <span className="bg-[#121317] px-1 py-0.5 rounded border border-[#22252C] font-mono">Ctrl+K</span> to close</span>
                </div>
              )}
            </div>
          </motion.div>
