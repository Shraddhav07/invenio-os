...
export const Sidebar: React.FC = () => {
  const { activeRoute, setRoute } = useInvenioStore();
      ...
      <div className="p-5 flex items-center space-x-3 border-b border-[#22252C]">
        ...
        <div>
          <h1 className="text-sm font-bold tracking-wider text-white m-0 leading-none">
            INVENIO{" "}
            <span className="text-[#FF6B35] font-semibold text-xs">OS</span>
          </h1>
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-3 z-10">
                <Icon
                  className={`w-4 h-4 ${isActive ? "text-[#FF6B35]" : "text-zinc-500"}`}
                />
                <span>{item.label}</span>
              </div>

              {item.id === "alerts" && <AlertBadge />}
...
const AlertBadge: React.FC = () => {
  const alertsCount = useInvenioStore(
    (state) =>
