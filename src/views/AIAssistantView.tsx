...
import { Send, BrainCircuit, Terminal, Sparkles, AlertTriangle, User } from 'lucide-react'
...
export const AIAssistantView: React.FC = () => {
  ...
  const addMessage = (sender: 'user' | 'assistant', text: string, customContent?: React.ReactNode) => {
  ...
  const triggerFailover = (queryText: string, reason: string) => {
    ...
    const misplaced = inventory.filter(item => item.status === 'error' || item.assignedShelf !== item.currentShelf)
  ...
  const executeCommand = async (query: string) => {
    ...
    setThinkingLogs(['Analyzing warehouse data...', 'Interfacing with physical warehouse registry...'])

    // Pronoun / Session Memory Resolver:
    let resolvedQuery = query.toLowerCase().trim()
    const referencesIt = resolvedQuery.includes(' it') || resolvedQuery.includes(' that') || resolvedQuery.includes(' fix it') || resolvedQuery.includes(' show it') || resolvedQuery.includes(' show me that') || resolvedQuery.includes(' locate it') || resolvedQuery.includes(' where is it') || resolvedQuery.includes(' track it') || resolvedQuery.includes(' follow it') || resolvedQuery.includes(' assigned to it') || resolvedQuery.includes(' assigned to fix it')
    ...
    if (referencesIt && messages.length > 0) {
      for (let i = messages.length - 1; i >= 0; i--) {
        ...
        if (text.includes('alpha')) {
          resolvedQuery = resolvedQuery.replace(/\b(it|him|that|worker)\b/g, 'alpha')
          break
        }
        if (text.includes('beta')) {
          resolvedQuery = resolvedQuery.replace(/\b(it|him|that|worker)\b/g, 'beta')
          break
        }
        
        const foundItem = inventory.find(item => text.includes(item.name.toLowerCase()))
        if (foundItem) {
          resolvedQuery = resolvedQuery.replace(/\b(it|that|them|item)\b/g, foundItem.name.toLowerCase())
          break
        }

        const shelfMatch = msg.text.match(/\b([A-Za-z]\d+)\b/)
        if (shelfMatch) {
          resolvedQuery = resolvedQuery.replace(/\b(it|that|shelf)\b/g, `shelf ${shelfMatch[1]}`)
          break
        }
      }
    }
    ...
    if (cacheRef.current[cleanQ]) {
      setTimeout(() => {
        setThinkingLogs(prev => [...prev, 'Reading response cache...', 'Correlating matching records...'])
      }, 200)
      setTimeout(() => {
        setIsThinking(false)
    ...
    const isLocalQuery = 
      q.includes('add') || q.includes('register') || q.includes('create') ||
      q.includes('update') || q.includes('change') || q.includes('set') ||
      q.includes('remove') || q.includes('delete') || q.includes('reduce') || q.includes('subtract') ||
      q.includes('where') || q.includes('find') || q.includes('locate') || q.includes('search') || q.includes('show') || q.includes('coordinate') ||
      q.includes('movement') || q.includes('log') || q.includes('history') || q.includes('activity') ||
      q.includes('stock') || q.includes('quantity') || q.includes('how many') ||
      q.includes('restock') || q.includes('reorder') || q.includes('suggest') ||
      q.includes('report') || q.includes('audit') || q.includes('summary') || q.includes('status') || q.includes('count') ||
      q.includes('misplaced') || q.includes('anomaly') || q.includes('wrong') ||
      q.includes('optimize') || q.includes('layout') || q.includes('safety') ||
      q.includes('follow') || q.includes('track') || q.includes('alpha') || q.includes('beta')

    if (isLocalQuery) {
      setTimeout(() => {
        setThinkingLogs(prev => [...prev, 'Correlating inventory item records...'])
      }, 300)
      ...
      setTimeout(() => {
        ...
        // 1. ADD Command
        const addMatchWithShelf = query.match(/(?:add|register|create)\s+(\d+)\s+([\w\s]+)\s+(?:to|at|in)\s+shelf\s+([A-Za-z]\d+)/i)
        const addMatchWithZone = query.match(/(?:add|register|create)\s+(\d+)\s+([\w\s]+)\s+(?:to|at|in)\s+(Zone\s+[A-C]|Hazard\s+Storage|Cold\s+Storage)/i)
        const addMatchSimple = query.match(/(?:add|register|create)\s+(\d+)\s+([\w\s]+)/i)
        ...
        if (addMatchWithShelf) {
          ...
          const responseText = addInventoryAI(itemName, qty, category, zone, shelf)
          if (responseText) {
            addMessage('assistant', `Registered ${qty} units of ${itemName} to Shelf ${shelf} (${zone}) successfully.\n\n💡 Recommendation\nCheck the Digital Twin view to monitor worker pathing to the shelf.`)
            return
          }
          ...
          const existing = inventory.find(i => i.name.toLowerCase().includes(itemName.toLowerCase()))
          const shelf = existing ? existing.assignedShelf : getDefaultShelfForZone(zoneInput)
          const category = existing ? existing.category : getCategoryFromShelf(shelf)
          const zone = getZoneFromShelf(shelf)

          const responseText = addInventoryAI(itemName, qty, category, zone, shelf)
          if (responseText) {
            addMessage('assistant', `Registered ${qty} units of ${itemName} to Zone ${zoneInput} (Shelf ${shelf}) successfully.\n\n💡 Recommendation\nCheck the Digital Twin view to monitor worker pathing to the shelf.`)
            return
          }
        } else if (addMatchSimple && !q.includes('to shelf') && !q.includes('at shelf') && !q.includes('to zone')) {
          ...
          const responseText = addInventoryAI(itemName, qty, category, zone, shelf)
          if (responseText) {
            addMessage('assistant', `Registered ${qty} units of ${itemName} to shelf ${shelf} successfully.\n\n💡 Recommendation\nCheck the Digital Twin view to monitor worker pathing to the shelf.`)
            return
          }
        }

        // 2. UPDATE Command
        const updateMatch = query.match(/(?:update|change|set)\s+([\w\s]+)\s+quantity\s+to\s+(\d+)/i) || 
        ...
        if (updateMatch) {
          ...
          if (responseText && !responseText.includes('not found')) {
            addMessage('assistant', `Set quantity of ${itemName} to ${qty} units in registry successfully.\n\n💡 Recommendation\nEnsure that physical shelves are verified to match this stock adjustment.`)
            return
          } else {
            addMessage('assistant', `Could not update: No item matching ${itemName} was found in active inventory.`)
            return
          }
        }

        // 3. REMOVE Command
        const removeMatch = query.match(/(?:remove|delete|reduce|subtract)\s+(\d+)\s+([\w\s]+)/i)
        if (removeMatch) {
          ...
          if (responseText && !responseText.includes('not found')) {
            addMessage('assistant', `Removed ${qty} units of ${itemName} from warehouse stock.\n\n💡 Recommendation\nVerify the shelf capacity matches the remaining quantity.`)
            return
          } else {
            addMessage('assistant', `Failed to remove: Item ${itemName} was not found, or quantity is insufficient.`)
            return
          }
        }
        ...
        if (q.includes('follow') || q.includes('track')) {
          const workerId = q.includes('beta') ? 'beta' : q.includes('alpha') ? 'alpha' : null
          if (workerId) {
            setFollowingWorkerId(workerId)
            addMessage('assistant', `Now tracking ${workerId === 'alpha' ? 'Alpha Forklift' : 'Beta Forklift'} telemetry.\n\n💡 Recommendation\nUse the camera controls to follow the worker's movement.`)
            setTimeout(() => {
              setRoute('twin')
            }, 600)
            return
        ...
        if (q.includes('what') && (q.includes('alpha') || q.includes('beta') || q.includes('worker') || q.includes('forklift'))) {
        ...
        const locateWords = ['find', 'locate', 'where', 'search', 'show', 'coordinate']
        if (locateWords.some(word => q.includes(word))) {
          let searchName = q
          locateWords.forEach(w => {
            searchName = searchName.replace(w, '')
          })
          searchName = searchName.replace(/(?:are|is|the|for|item|unit|units|\?)/g, '').trim()

          if (searchName) {
            const match = inventory.find(item => item.name.toLowerCase().includes(searchName))
            if (match) {
        ...
        if (q.includes('movement') || q.includes('log') || q.includes('history') || q.includes('activity')) {
        ...
        if (q.includes('stock') || q.includes('quantity') || q.includes('how many')) {
          let searchName = q.replace(/(?:check|stock|levels|level|of|quantity|how|many|units|do|we|have|\?)/g, '').trim()
          if (searchName) {
            const match = inventory.find(item => item.name.toLowerCase().includes(searchName))
            if (match) {
              addMessage('assistant', `Stock check: Found ${match.quantity} units of ${match.name}.\n\n• Total Inventory: ${match.quantity} units\n• Shelf: ${match.currentShelf}\n\n💡 Recommendation\nVerify replenishment schedule if levels fall below safety thresholds.`)
              return
            }
          }
        ...
        if (q.includes('restock') || q.includes('reorder') || q.includes('suggest')) {
          const lowStock = inventory.filter(item => item.quantity < 50)
        ...
        if (q.includes('report') || q.includes('audit')) {
          const totalItems = inventory.reduce((acc, curr) => acc + curr.quantity, 0)
          const verifiedShelves = Object.values(useInvenioStore.getState().shelves).filter(s => s.status === 'verified').length
          const totalShelves = Object.keys(useInvenioStore.getState().shelves).length
          const activeAlerts = alerts.filter(a => !a.resolved).length
      ...
      try {
        ...
        const warehouseContext = {
          ...
          alerts: safeAlerts,
          kpi: {
            totalInventoryQty: safeInventory.reduce((acc, curr) => acc + curr.quantity, 0),
            accuracy: storeState.inventory.length > 0 
              ? ((1 - (storeState.inventory.filter(item => item.status === 'error' || item.assignedShelf !== item.currentShelf).length / storeState.inventory.length)) * 100).toFixed(1)
              : '100.0',
            activeAlertsCount: safeAlerts.filter(a => !a.resolved).length,
            activeWorkersCount: safeWorkers.filter(w => w.status !== 'idle').length
          }
        }
  ...
  const handleTrigger = (type: 'misplaced' | 'optimize' | 'summary') => {
    ...
    setIsThinking(true)
    setShowFailoverWarning(false)
    setThinkingLogs(['Initiating spatial audit...', 'Fetching spatial data streams...'])
    ...
    setTimeout(() => {
      ...
      if (type === 'misplaced') {
        ...
        const content = (
          <div className="mt-2 space-y-3">
            <p className="text-[10px] text-[#A1A1AA]">
              I identified <strong>{misplaced.length}</strong> misplaced item(s):
            </p>
            <div className="border border-[#22252C] rounded-lg overflow-hidden bg-[#0B0B0D]">
              <table className="w-full text-left text-[10px] border-collapse">
                ...
                <tbody className="divide-y divide-[#22252C]">
                  {misplaced.map(item => (
                    <tr key={item.sku} className="hover:bg-[#171A20]/30 text-white">
                      <td className="p-2 font-medium">{item.name}</td>
                      <td className="p-2 font-mono text-zinc-500">{item.assignedShelf}</td>
                      <td className="p-2 font-mono text-[#EF4444] font-bold">{item.currentShelf}</td>
        ...
        addMessage('assistant', `Spatial scanning complete. Found ${misplaced.length} placement anomalies.`, content)
      
      } else if (type === 'optimize') {
        const content = (
          <div className="mt-2 space-y-3">
            <p className="text-[10px] text-[#A1A1AA]">
              Current layout recommendations based on safety codes and zoning rules:
            </p>
            <div className="space-y-2">
              {recommendations.map((rec) => (
                <div key={rec.id} className="p-3 bg-[#171A20] border border-[#22252C] rounded-lg flex flex-col space-y-1">
                  <div className="flex justify-between items-center">
                    <span className="text-[11px] font-bold text-white">{rec.title}</span>
                    <span className="text-[9px] font-mono text-[#FF6B35] uppercase font-semibold">{rec.impact}</span>
                  </div>
                  <p className="text-[10px] text-[#A1A1AA] leading-normal">{rec.description}</p>
                </div>
              ))}
            </div>
          </div>
        )
        addMessage('assistant', "I have calculated layout optimizations to isolate hazards and streamline picking paths.", content)
      
      } else if (type === 'summary') {
        const okShelves = Object.values(useInvenioStore.getState().shelves).filter(s => s.status === 'verified').length
        const totalShelves = Object.keys(useInvenioStore.getState().shelves).length
        const activeAlerts = alerts.filter(a => !a.resolved).length
        ...
        const content = (
          <div className="mt-2 grid grid-cols-2 gap-3.5">
            <div className="p-3 bg-[#171A20] border border-[#22252C] rounded-lg">
              <span className="text-[9px] uppercase tracking-wider text-[#A1A1AA] block">Verified Shelves</span>
              <span className="text-sm font-extrabold text-white mt-1 font-mono block">{okShelves} / {totalShelves} OK</span>
            </div>
            <div className="p-3 bg-[#171A20] border border-[#22252C] rounded-lg">
              <span className="text-[9px] uppercase tracking-wider text-[#A1A1AA] block">System Alerts</span>
              <span className="text-sm font-extrabold text-[#EF4444] mt-1 font-mono block">{activeAlerts} Active</span>
            </div>
          </div>
        )
        addMessage('assistant', `Warehouse report ready. Operations are executing within normal tolerances.`, content)
      }
    }, 900)
  }
  ...
  const parseAIResponseToComponents = (text: string) => {
        ...
        components.push(
          <ul key={key} className="space-y-1.5 my-2.5">
            {currentBulletList.map((bullet, bIdx) => (
              <li key={bIdx} className="flex items-start space-x-2 text-[11px] text-zinc-300 leading-normal">
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (isHeader) {
        ...
        if (line.includes('📦') || line.toLowerCase().includes('inventory')) icon = '📦'
        else if (line.includes('⚠️') || line.toLowerCase().includes('issue') || line.toLowerCase().includes('critical')) icon = '⚠️'
        else if (line.includes('💡') || line.toLowerCase().includes('recommendation') || line.toLowerCase().includes('action')) icon = '💡'
        else if (line.includes('📈') || line.toLowerCase().includes('performance') || line.toLowerCase().includes('accuracy')) icon = '📈'
        else if (line.includes('🚜') || line.toLowerCase().includes('worker') || line.toLowerCase().includes('alpha') || line.toLowerCase().includes('beta')) icon = '🚜'
        else if (line.includes('📊') || line.toLowerCase().includes('report') || line.toLowerCase().includes('dashboard')) icon = '📊'
        ...
        if (icon === '💡') {
          components.push(
            <div key={`rec-card-${i}`} className="my-3.5 p-3.5 bg-[#FF6B35]/5 border border-[#FF6B35]/25 rounded-xl flex items-start space-x-3.5">
              <span className="text-base shrink-0">💡</span>
              <div className="flex-1 space-y-1">
                <span className="text-xs font-bold text-white uppercase tracking-wider block">Recommendation</span>
                <p className="text-[11px] text-[#A1A1AA] leading-normal">{cleanHeaderText}</p>
              </div>
            </div>
          )
        } else if (icon === '⚠️') {
          components.push(
            <div key={`warn-card-${i}`} className="my-3.5 p-3.5 bg-[#EF4444]/5 border border-[#EF4444]/25 rounded-xl flex items-start space-x-3.5">
              <span className="text-base shrink-0">⚠️</span>
              <div className="flex-1 space-y-1">
                <span className="text-xs font-bold text-[#EF4444] uppercase tracking-wider block">Key Issues / Alert Details</span>
                <p className="text-[11px] text-[#A1A1AA] leading-normal">{cleanHeaderText}</p>
              </div>
            </div>
          )
          ...
          components.push(
            <h4 key={`header-${i}`} className="text-xs font-extrabold text-white mt-4 mb-2 flex items-center space-x-2 border-b border-[#22252C]/30 pb-1 uppercase tracking-wider">
      ...
      if (line.startsWith('*') || line.startsWith('-') || line.startsWith('•')) {
        ...
        const badgeMatch = bulletText.match(/(?:🔴|🟢|📦|📈)?\s*(Critical Alerts|Verified Items|Total Inventory|Inventory Accuracy|System Mode|Verification Rate|Active Alerts|Total Registry Count|Placement Integrity|Critical Incidents|Fleet Availability|Total Items|Verified Shelves|System Alerts):\s*(.+)/i)
        ...
        if (badgeMatch) {
          ...
          let indicatorIcon = '📋'
          if (label.toLowerCase().includes('critical') || label.toLowerCase().includes('alert') || label.toLowerCase().includes('incident')) {
            colorClass = 'bg-[#EF4444]/10 text-[#EF4444] border-[#EF4444]/25'
            indicatorIcon = '🔴'
          } else if (label.toLowerCase().includes('verified') || label.toLowerCase().includes('accuracy') || label.toLowerCase().includes('rate')) {
            colorClass = 'bg-[#22C55E]/10 text-[#22C55E] border-[#22C55E]/25'
            indicatorIcon = '🟢'
          } else if (label.toLowerCase().includes('inventory') || label.toLowerCase().includes('registry') || label.toLowerCase().includes('total')) {
            colorClass = 'bg-[#FF6B35]/10 text-[#FF6B35] border-[#FF6B35]/25'
            indicatorIcon = '📦'
          }
          
          components.push(
            <div key={`badge-${i}`} className={`inline-flex items-center space-x-2 px-3 py-1.5 rounded-lg border text-[10px] font-mono font-bold mr-2 my-1.5 ${colorClass}`}>
              <span>{indicatorIcon}</span>
              <span className="uppercase tracking-wider opacity-85">{label}:</span>
      ...
      components.push(
        <p key={`p-${i}`} className="text-[11px] text-zinc-400 leading-relaxed my-2">
          {cleanText}
        </p>
      )
    }
    ...
    return components.length > 0 ? components : <div className="text-[11px] text-zinc-400">{text}</div>
  }
  ...
  const renderActionButtons = (msg: Message) => {
    ...
    const matchedItem = inventory.find(item => textLower.includes(item.name.toLowerCase()))
    if (matchedItem) {
      buttons.push(
        <button
          key="locate-item"
      ...
      buttons.push(
        ...
        >
          <span>📂 Open Shelf {matchedItem.currentShelf || matchedItem.assignedShelf}</span>
        </button>
      )
      buttons.push(
        <button
    ...
    // 2. Alerts Category
    if (textLower.includes('alert') || textLower.includes('misplaced') || textLower.includes('issue') || textLower.includes('anomaly') || textLower.includes('critical')) {
      buttons.push(
        <button
          key="view-alert"
    ...
    // 3. Workers Category
    if (textLower.includes('alpha') || textLower.includes('beta') || textLower.includes('forklift') || textLower.includes('worker')) {
      if (textLower.includes('alpha') || (!textLower.includes('beta') && !textLower.includes('alpha'))) {
        buttons.push(
          <button
            key="follow-alpha"
            onClick={() => {
      ...
      if (textLower.includes('beta') || (!textLower.includes('beta') && !textLower.includes('alpha'))) {
        buttons.push(
          <button
            key="follow-beta"
            onClick={() => {
    ...
    if (textLower.includes('report') || textLower.includes('summary') || textLower.includes('kpi') || textLower.includes('performance') || textLower.includes('audit') || textLower.includes('layout') || textLower.includes('optimize') || textLower.includes('analytics')) {
      buttons.push(
        <button
          key="export-report"
          onClick={() => {
  ...
  return (
    <div className="w-full h-full p-6 flex flex-col space-y-6 overflow-hidden bg-[#0B0B0D] text-white">
      ...
      <div className="flex-1 bg-[#121317] rounded-xl border border-[#22252C] p-4 flex flex-col justify-between overflow-hidden relative">
          ...
          {showFailoverWarning && (
            <div className="bg-[#FF6B35]/10 border border-[#FF6B35]/20 p-2.5 rounded-lg flex items-center justify-between text-[10px] text-zinc-350">
              <div className="flex items-center space-x-2">
                ...
                <span>Advanced AI temporarily unavailable. Using local warehouse intelligence.</span>
              </div>
              <button 
                type="button" 
          ...
          {messages.length === 0 && (
            <div className="max-w-2xl mx-auto my-6 p-6 bg-[#0B0B0D]/50 border border-[#22252C] rounded-2xl space-y-5">
              <div className="flex items-center space-x-3.5 pb-4 border-b border-[#22252C]">
                ...
                <div>
                  <h3 className="text-sm font-extrabold text-white">👋 Welcome to Invenio AI</h3>
                  <p className="text-[10px] text-[#A1A1AA] uppercase tracking-wider mt-0.5">Warehouse Operations Copilot</p>
                </div>
              </div>
              
              <div className="space-y-3">
                <span className="text-[10px] font-bold text-zinc-450 uppercase tracking-wider block">I can help you:</span>
              ...
              <div className="space-y-2.5 pt-2 border-t border-[#22252C]/40">
                <span className="text-[10px] font-bold text-zinc-455 uppercase tracking-wider block">Try asking:</span>
            ...
            >
              ...
              <div className={`p-3 rounded-xl border text-xs leading-relaxed ${
                ...
                {msg.sender === 'user' ? (
                  ...
                  <div className="space-y-1">{parseAIResponseToComponents(msg.text)}</div>
                )}
                {msg.customContent && <div className="mt-1">{msg.customContent}</div>}
        ...
        {/* Chat input box */}
        <form onSubmit={handleSubmit} className="flex items-center space-x-3 bg-[#171A20] p-2 rounded-xl border border-[#22252C]">
          <input 
            type="text" 
            value={input}
