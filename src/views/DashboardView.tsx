...
export const DashboardView: React.FC = () => {
  ...
  const misplacedItems = inventory.filter(
    (item) =>
  ...
  const pendingVerifications = Object.values(shelves).filter(
    (s) => s.status === "pending",
  ).length;
  ...
  const generateInsights = () => {
    ...
    if (misplacedItems.length > 0) {
      ...
      list.push(
        `${first.name} misplaced on shelf ${first.currentShelf} instead of ${first.assignedShelf}.`,
      );
    } else {
      list.push(
        `All shelf placements verified. No misplaced anomalies detected.`,
      );
    }

    list.push(
      `Zone B utilization reached 92%. Plan restocking layout optimization.`,
    );
    ...
    if (lowStockItems.length > 0) {
      ...
      list.push(
        `${firstLow.name} may run out within 5 days (${firstLow.quantity} units left).`,
      );
    } else {
    ...
    list.push(
      `Cold Storage utilization increased by 12% due to new medical vaccine arrivals.`,
    );
        ...
        >
          <div className="space-y-1">
            ...
            <span className="text-[8px] text-[#EF4444]">
              {criticalAlertsCount > 0 ? "Anomaly found" : "Healthy status"}
            </span>
          </div>
            ...
            {insights.map((insight, idx) => (
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[9px] font-extrabold text-zinc-300 uppercase tracking-wider">
                    Insight #{idx + 1}
                  </span>
                </div>
          ...
          <div className="flex items-center justify-between border-b border-[#22252C] pb-3 mb-4">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-[10px] font-bold text-white uppercase tracking-wider">
                AI Inventory Forecasting & Capacity Planning
              </span>
            </div>
            <span className="text-[9px] text-[#22C55E] font-medium uppercase tracking-wider bg-[#22C55E]/10 px-2 py-0.5 rounded-full">
              Stabilized Trend
            </span>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 text-center">
                        <span
                          className={`px-2 py-0.5 rounded-md text-[8px] font-extrabold uppercase font-mono ${
                            isCritical
          ...
          <div className="bg-[#121317] border border-[#22252C] rounded-xl p-5 flex flex-col space-y-3.5">
            ...
            <div className="grid grid-cols-2 gap-3.5 pt-1">
              <div className="bg-[#171A20] p-3 rounded-lg border border-[#22252C]">
                <span className="text-[8px] text-[#A1A1AA] uppercase tracking-wider block">
                  Jobs Completed
                </span>
              ...
              <div className="bg-[#171A20] p-3 rounded-lg border border-[#22252C] col-span-2">
                {(() => {
                  const recItems = inventory.filter(
                    (item) => item.currentShelf === "REC",
                  );
                  const recSlotsUsed = recItems.reduce(
                    ...
                    <>
                      ...
                      <div className="w-full bg-[#121317] h-1.5 rounded overflow-hidden mt-1.5 border border-[#22252C]">
                        <div
                          ...
                          style={{
                            width: `${Math.min(100, (recSlotsUsed / 20) * 100)}%`,
                          }}
                        />
                      </div>
                    </>
            ...
            <div>
              ...
              <div className="space-y-3 max-h-52 overflow-y-auto pr-1">
                {activities.slice(0, 10).map((act) => (
                  <div
                    key={act.id}
                    className="flex items-start justify-between text-[10px] border-b border-[#22252C]/30 pb-2 last:border-0 last:pb-0"
                  >
            ...
            >
              ...
              <div className="p-5 border-b border-[#22252C] flex items-center justify-between bg-[#16171D]">
                <div>
                  ...
                  <p className="text-[10px] text-zinc-400 mt-1">
                    Search, filter, and dispatch AGV relocation missions
                  </p>
                </div>
                <button
                  onClick={() => setDrawerOpen(false)}
              ...
              <div className="p-4 border-b border-[#22252C] bg-[#14151A] space-y-3">
                ...
                <div className="flex bg-zinc-900/60 p-0.5 rounded border border-zinc-800/80 text-[10px]">
                  {(["all", "pending", "active", "completed"] as const).map(
                    (f) => {
                      const count =
                        f === "all"
                          ? taskQueue.length
                          : taskQueue.filter((t) => t.status === f).length;
                      return (
                        <button
                          key={f}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {(() => {
                  const filtered = taskQueue.filter((t) => {
                    if (drawerFilter !== "all" && t.status !== drawerFilter)
                      return false;
                  ...
                  return filtered.map((task) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-[10px] text-zinc-500 pt-1 border-t border-zinc-800/40">
                        <span>
                          Type:{" "}
                          <strong className="text-zinc-400 capitalize">
                            {task.type}
                          </strong>
                        </span>
                        <span>
                          AGV:{" "}
                          <strong className="text-zinc-400 uppercase">
