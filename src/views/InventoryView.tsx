...
import { Search, Plus, Navigation, AlertTriangle, CheckCircle, HelpCircle, Shield, MapPin } from 'lucide-react'
import { useInvenioStore, type InventoryItem, isShelfAllowedForCategory, getRecommendedZones } from '../store/store'

export const InventoryView: React.FC = () => {
  const { inventory, locateItem, simulateNewArrival, shelves } = useInvenioStore()
  ...
  const getFilteredInventory = () => {
    ...
    if (shelfFilter !== 'All') {
      items = items.filter(item => item.currentShelf === shelfFilter || item.assignedShelf === shelfFilter)
    }
    ...
    if (query === 'misplaced' || query === 'error' || query === 'wrong placement' || query === 'anomaly') {
      return items.filter(item => item.status === 'error' || item.assignedShelf !== item.currentShelf)
    }

    // Semantic trigger: categories or terms
    if (['beverages', 'food', 'electronics', 'furniture', 'industrial', 'medical', 'cleaning', 'hazardous'].some(cat => query.includes(cat))) {
              ...
              <table className="w-full border-collapse text-left text-xs">
                <thead>
                  <tr className="border-b border-[#22252C] bg-[#171A20] text-[#A1A1AA] font-semibold tracking-wider uppercase">
                    ...
                    <th className="p-4 text-[10px]">Location (Assigned / Scanned)</th>
                ...
                <tbody className="divide-y divide-[#22252C] font-medium bg-[#121317]">
                  {filteredItems.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="p-8 text-center text-[#A1A1AA] text-xs">
                        No matching inventory SKUs found.
                      </td>
                    </tr>
                  ) : (
                    filteredItems.map((item) => {
                      const isMisplaced = item.assignedShelf !== item.currentShelf && item.status === 'error'
                      const isNotAllowed = !isShelfAllowedForCategory(item.category, item.currentShelf, shelves)
                      return (
                        <tr 
                          key={item.sku} 
                        ...
                        >
                          ...
                          <td className="p-4 font-mono text-[10px]">
                            <div className="flex items-center space-x-1.5">
                              <span className="text-zinc-400">{item.assignedShelf}</span>
                              <span className="text-zinc-600">→</span>
                              <span className={isMisplaced || isNotAllowed ? 'text-[#EF4444] font-bold' : 'text-zinc-500'}>
                                {item.currentShelf}
                              </span>
                            </div>
                          </td>
                            ...
                            <div className="flex items-center space-x-1.5">
                              ...
                              {item.status === 'pending' && (
                                <>
                                  ...
                                  <span className="text-[#F59E0B] text-[10px]">Pending</span>
                                </>
                              )}
                          ...
                          <td className="p-4 text-right">
                            ...
                            >
                              ...
                              <span className="font-semibold uppercase">Locate</span>
                            </button>
                          </td>
                        </tr>
                      )
        ...
        <div className="lg:col-span-3 flex flex-col space-y-6">
          <div className="bg-[#121317] p-5 rounded-xl border border-[#22252C] flex flex-col space-y-4">
            <div className="flex items-center space-x-2 pb-3 border-b border-[#22252C]">
              ...
              <h3 className="text-xs font-bold text-white uppercase tracking-wider">Zone Safety Directives</h3>
            </div>

            <p className="text-[10px] text-zinc-400 leading-relaxed">
              Strict rules guide the shelf mapping of inventory items by category to satisfy hazard containment and cooling directives:
            </p>

            <div className="space-y-3 pt-1">
              <div className="p-2.5 bg-[#171A20] border border-[#22252C] rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-[#FF6B35] uppercase">Zone A (General)</span>
                  <span className="text-[9px] font-mono text-[#A1A1AA]">A1-A3</span>
                </div>
                <span className="text-[9px] text-zinc-400 block mt-1">Allowed: Electronics, Furniture, Industrial</span>
              </div>

              <div className="p-2.5 bg-[#171A20] border border-[#22252C] rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-[#FF6B35] uppercase">Zone B (Ambient)</span>
                  <span className="text-[9px] font-mono text-[#A1A1AA]">B1-B3</span>
                </div>
                <span className="text-[9px] text-zinc-400 block mt-1">Allowed: Food, Cleaning Supplies, Medical</span>
              </div>

              <div className="p-2.5 bg-[#171A20] border border-[#22252C] rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-[#FF6B35] uppercase">Zone C (High-Qty)</span>
                  <span className="text-[9px] font-mono text-[#A1A1AA]">C1-C3</span>
                </div>
                <span className="text-[9px] text-zinc-400 block mt-1">Allowed: Beverages, General Merch</span>
              </div>

              <div className="p-2.5 bg-[#171A20] border border-[#22252C] rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-red-400 uppercase">Hazardous Storage</span>
                  <span className="text-[9px] font-mono text-[#A1A1AA]">H1-H2</span>
                </div>
                <span className="text-[9px] text-zinc-400 block mt-1">Allowed: Hazardous chemicals, batteries</span>
              </div>

              <div className="p-2.5 bg-[#171A20] border border-[#22252C] rounded-lg">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-sky-400 uppercase">Cold Storage</span>
                  <span className="text-[9px] font-mono text-[#A1A1AA]">F1-F2</span>
                </div>
                <span className="text-[9px] text-zinc-400 block mt-1">Allowed: Refrigerated medical, perishables</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="w-full max-w-md bg-[#121317] p-6 rounded-xl border border-[#22252C] shadow-2xl flex flex-col space-y-4">
            <div className="flex items-center space-x-2 pb-2 border-b border-[#22252C]">
              ...
              <h3 className="text-xs font-bold text-white uppercase tracking-wider m-0">Register Catalog SKU</h3>
            </div>
            
            <form onSubmit={handleAddNewItem} className="space-y-4 text-xs text-[#A1A1AA]">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col space-y-1">
                  <label className="text-zinc-400 font-medium">Zone Allocation</label>
                  <input
                    type="text"
                    value={zone}
                    disabled
              ...
              {!allowedForForm && (
                <div className="p-2.5 bg-[#EF4444]/10 border border-[#EF4444]/30 rounded text-[9px] text-[#EF4444] leading-relaxed">
                  <strong>Warning:</strong> Placing "{category}" on shelf "{shelf}" violates strict container safety directives. Please choose a shelf in: <strong>{suggestedForForm.join(', ')}</strong>.
                </div>
              )}
